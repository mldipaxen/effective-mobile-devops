...
8) Реализован graceful shutdown в Python-приложении (корректная обработка SIGTERM)
9) Healthchecks в Dockerfile и docker-compose.yml с настройкой интервала, таймаута и периода запуска
10) Зависимость depends_on: condition: service_healthy гарантирует запуск nginx только после готовности backend
11) Ограничение размера логов (ротация: 10 МБ на файл, максимум 3 файла)
